...
class InstallController extends Controller{
	...
	public function step2($db = null, $admin = null){
		if(IS_POST){
			...
			//检测数据库配置
			if(!is_array($db) || empty($db[0]) ||  empty($db[1]) || empty($db[2]) || empty($db[3])||empty($db[6])||empty($db[7])||empty($db[8])){
				$this->error('请填写完整的数据');
			} else {
				$DB = array();
				list($DB['DB_TYPE'], $DB['DB_HOST'], $DB['DB_NAME'], $DB['DB_USER'], $DB['DB_PWD'],
					 $DB['DB_PORT'], $DB['DB_PREFIX'],$DB['PROJECT_NAME'],$DB['SITE_URL'],$DB['ITBOYE_CDN']) = $db;
				//缓存数据库配置
